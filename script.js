...
// 🔥 ORIGINAL SIZE UPLOAD WITH PDF BYPASS
uploadInput.onchange = (e) => {
    const file = e.target.files[0];
    if (!file) return;
    
    // PDF BYPASS: Agar PDF hai toh image block mat chalao
    if (file.type === 'application/pdf') {
        console.log("PDF File Detected! Handled by PDF Engine.");
        return; 
    }
    ...
    reader.onload = (event) => {
        img = new Image();
        img.onload = () => {
            // Hamesha original dimension set karega (No quality loss)
...
function handlePointerDown(e) {
    ...
    // Sirf canvas ke upar scroll block karo, baaki screen scroll ho sake
...
function drawEverything() {
    ...
    // Hamesha Image asli size me draw hogi
